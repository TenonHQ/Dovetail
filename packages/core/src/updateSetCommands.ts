...
export async function showCurrentScopeCommand(args: any): Promise<void> {
  ...
  try {
    ...
    if (scopeObj && scopeObj.scope) {
      ...
      logger.info(
        chalk.green(`► ${(scopeObj as any).displayName || scopeObj.scope}`),
      );
  ...
  try {
    ...
    if (e instanceof Error) {
      ...
      if ((e as any).response) {
        ...
        logger.error(
          `Response data: ${JSON.stringify((e as any).response.data)}`,
        );
      }
    }
    throw e;
...
export async function showCurrentUpdateSetCommand(args: any): Promise<void> {
  ...
  try {
    ...
    if (result && result.sysId) {
      ...
      logger.warn(
        "No update set is currently active or unable to retrieve update set information",
      );
    }
  } catch (e) {
...
export async function createUpdateSetCommand(args: any): Promise<void> {
  ...
  try {
    ...
    const createResponse = client.createUpdateSet(
      name,
      scopeSysId,
      description,
    );
    ...
    try {
      ...
      logger.warn(
        `Update set "${name}" created but could not be activated automatically`,
      );
      logger.info(
        `You can manually switch to it using: npx dove switchUpdateSet --name "${name}"`,
      );
          ...
          var commentAnswer = await inquirer.prompt([
            {
              ...
              name: "postComment",
              message:
                "Post a comment to the ClickUp task linking this update set?",
              default: true,
            },
          ]);
...
export async function switchUpdateSetCommand(args: any): Promise<void> {
  ...
  try {
    ...
    const sysId =
      typeof targetUpdateSet.sys_id === "object" &&
      targetUpdateSet.sys_id !== null
        ? (targetUpdateSet as any).sys_id.value
        : targetUpdateSet.sys_id;
    const name =
      typeof targetUpdateSet.name === "object" && targetUpdateSet.name !== null
        ? (targetUpdateSet as any).name.value
        : targetUpdateSet.name;
    const applicationName =
      (targetUpdateSet.application &&
        targetUpdateSet.application.display_value) ||
      (typeof targetUpdateSet.application === "object" &&
      targetUpdateSet.application !== null
        ? (targetUpdateSet as any).application.value
        : targetUpdateSet.application);
    ...
    if (scopeName) {
      ...
      if (routed) {
        logger.info(
          chalk.green(
            `✓ Push routing updated: ${scopeName} -> ${name} (${sysId})`,
          ),
        );
      }
    } else {
...
export async function listUpdateSetsCommand(args: any): Promise<void> {
  ...
  try {
    ...
    updateSets.forEach((updateSet: any) => {
      ...
      const sysId =
        typeof updateSet.sys_id === "object" && updateSet.sys_id !== null
          ? updateSet.sys_id.value
          : updateSet.sys_id;
      const name =
        typeof updateSet.name === "object" && updateSet.name !== null
          ? updateSet.name.value
          : updateSet.name;
      const description =
        typeof updateSet.description === "object" &&
        updateSet.description !== null
          ? updateSet.description.value
          : updateSet.description;
      const createdBy =
        typeof updateSet.sys_created_by === "object" &&
        updateSet.sys_created_by !== null
          ? updateSet.sys_created_by.display_value ||
            updateSet.sys_created_by.value
          : updateSet.sys_created_by;
      const applicationName =
        (updateSet.application && updateSet.application.display_value) ||
        (typeof updateSet.application === "object" &&
        updateSet.application !== null
          ? updateSet.application.value
      ...
      console.log(
        `    Created: ${formatDate(updateSet.sys_created_on)} by ${createdBy}`,
      );
 ...
 */
async function switchToUpdateSet(
  ...
  logger.debug(
    `Switching to update set - sysId: ${updateSetSysId}, name: ${name}, scope: ${scope}`,
  );
  ...
  if (!verified) {
    ...
    var retryVerified = await verifyActiveUpdateSet(
      client,
      updateSetSysId,
      scope,
    );
 ...
 */
async function verifyActiveUpdateSet(
  client: ReturnType<typeof defaultClient>,
  ...
  try {
    ...
    logger.warn(
      "Verification mismatch: expected " +
        expectedSysId +
        ", got " +
        (result && result.sysId ? result.sysId : "null"),
    );
    return false;
  } catch (e) {
    logger.warn(
 ...
 */
async function getActiveUpdateSetName(
  client: ReturnType<typeof defaultClient>,
  scope?: string,
): Promise<string | null> {
  try {
...
async function getUpdateSets(query: string): Promise<UpdateSetListResponse[]> {
  ...
  const response = client.client.get<
    Sinc.SNAPIResponse<UpdateSetListResponse[]>
  >(endpoint, {
    params: {
      sysparm_query: query,
      sysparm_fields:
        "sys_id,name,description,state,application,application.name,sys_created_on,sys_created_by",
      sysparm_limit: 100,
 ...
 */
async function getCurrentUpdateSetDetails(
  scope?: string,
): Promise<{ sysId: string; name: string } | null> {
  try {
    const client = defaultClient();
    const response = await client.getCurrentUpdateSet(scope);
    let result = await response.data;
...
async function selectUpdateSet(
  ...
  if (nameFilter) {
    filteredSets = updateSets.filter((us) => {
      const name =
        typeof us.name === "object" && us.name !== null
          ? (us as any).name.value
          : us.name;
  ...
  const choices = filteredSets.map((us) => {
    const name =
      typeof us.name === "object" && us.name !== null
        ...
        : us.name;
    const description =
      typeof us.description === "object" && us.description !== null
        ? (us as any).description.value
        : us.description;
    const applicationName =
      (us.application && us.application.display_value) ||
      (typeof us.application === "object" && us.application !== null
        ? (us as any).application.value
        : us.application);
    return {
  ...
  try {
    const actualDateString =
      typeof dateString === "object" && dateString !== null
        ? dateString.display_value || dateString.value
        : dateString;
