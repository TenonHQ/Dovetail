# @tenonhq/sincronia-mcp

MCP server exposing **read-only** ClickUp / Gmail / Google Calendar / ServiceNow
tools backed by the existing Sincronia integration packages. Phase 1 — write
operations are deliberately out of scope (see PRD).

## Install

```bash
npm i @tenonhq/sincronia-mcp
```
...
| `SINC_MCP_TELEMETRY_PATH`   | Override `~/.sincronia-mcp/telemetry.jsonl` |

> **OAuth scope warning:** Sincronia's existing Google refresh tokens are
> issued with **write-capable** scopes (`gmail.modify`, `calendar`).
> sincronia-mcp enforces read-only at the handler level (we never import the
> upstream write functions and ESLint blocks new such imports), but the token
> itself can write Gmail/Calendar. Re-running `sincronia-google-auth` setup
...
npx @tenonhq/sincronia-mcp

# Smoke check — list tools and exit, no transport
npx @tenonhq/sincronia-mcp --smoke
```
...
```jsonc
{
  "mcpServers": {
    "sincronia": {
      "command": "node",
      "args": ["/absolute/path/to/Sincronia/packages/sincronia-mcp/dist/server.js"]
    }
  }
}
```
...
- `task-manager.md`: "ClickUp `clickup_*` MCP tools (under the `sincronia`
...
- `cto-briefer.md`: "Live data: `gmail_*`, `calendar_*`, `clickup_*` MCP tools
  on the `sincronia` server. Prefer them over the cron'd context markdown when
  freshness matters."
- `decision-advisor.md`: "Calendar / ClickUp MCP tools on the `sincronia`
  server are available for context-rich reasoning."

## Telemetry

Every tool call appends one JSON line to `~/.sincronia-mcp/telemetry.jsonl`
...
1. **Imports.** Tool modules import only the read functions from each upstream
   Sincronia package; write functions are never imported.
2. **ESLint** (`.eslintrc.json` `no-restricted-imports`) blocks import of any
   write function from `sincronia-clickup` / `sincronia-gmail` /
   `sincronia-google-calendar` / `sincronia-servicenow`.
...
- **"Google authentication failed (… ). Your refresh token may be expired or
  revoked."** Re-run `npm run setup` in `@tenonhq/sincronia-google-auth`.
