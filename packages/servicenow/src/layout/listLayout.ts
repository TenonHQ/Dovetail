...
import type { SetListLayoutParams, LayoutResult, LayoutRecordResult } from "../types";
import {
  assertUpdateSet,
  resolveScope,
  resolveView,
...
export async function setListLayout(
  ...
  if (list) {
    records.push({ table: "sys_ui_list", sysId: listSysId, action: "unchanged", label: params.table });
  } else if (dryRun) {
    records.push({ table: "sys_ui_list", sysId: "", action: "created", label: params.table });
  } else {
    ...
    records.push({ table: "sys_ui_list", sysId: listSysId, action: "created", label: params.table });
  }
  ...
  for (var p = 0; p < plan.length; p += 1) {
    ...
    if (step.action === "unchanged") {
      records.push({ table: "sys_ui_list_element", sysId: step.sysId, action: "unchanged", label: step.key });
    }
  }
  if (dryRun) {
    for (var d = 0; d < plan.length; d += 1) {
      var ds = plan[d];
      if (ds.action === "create") {
        records.push({ table: "sys_ui_list_element", sysId: "", action: "created", label: ds.key });
      } else if (ds.action === "update") {
        records.push({ table: "sys_ui_list_element", sysId: ds.sysId, action: "updated", label: ds.key });
      } else if (ds.action === "delete") {
        records.push({ table: "sys_ui_list_element", sysId: ds.sysId, action: "deleted", label: ds.key });
      }
    }
  ...
  for (var c = 0; c < plan.length; c += 1) {
    ...
    var created = await client.claude.createRecord({
      ...
      fields: { list_id: listSysId, element: plan[c].key, position: String(plan[c].position) },
      scope: scope,
      update_set_sys_id: params.updateSetSysId
    });
    records.push({ table: "sys_ui_list_element", sysId: created.sys_id, action: "created", label: plan[c].key });
  }
  for (var u = 0; u < plan.length; u += 1) {
    ...
    records.push({ table: "sys_ui_list_element", sysId: plan[u].sysId, action: "updated", label: plan[u].key });
  }
  ...
  if (deletes.length > 0) {
    ...
    for (var x = 0; x < deletes.length; x += 1) {
      await client.claude.deleteRecord({ table: "sys_ui_list_element", sys_id: deletes[x].sysId });
      records.push({ table: "sys_ui_list_element", sysId: deletes[x].sysId, action: "deleted", label: deletes[x].key });
    }
  }
