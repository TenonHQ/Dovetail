/**
 * setRelatedLists — declaratively reconcile a ServiceNow form's related lists.
 *
 * A related-list layout is a sys_ui_related_list row (keyed by table + view +
 * global sys_user) plus an ordered set of sys_ui_related_list_entry rows. The
 * caller supplies the desired ordered related-list identifiers; this upserts
 * the related list, then creates / repositions / deletes entry rows so the
 * layout matches — writing only the delta through the Dovetail REST API so
 * every change lands in the update set.
 */
...
export async function setRelatedLists(
  ...
  if (!params.relatedLists || params.relatedLists.length === 0) {
    throw new Error("relatedLists must be a non-empty array.");
  }
  var prune = params.prune !== false;
  var dryRun = params.dryRun === true;
  var relatedLists = dedupe(params.relatedLists);
  ...
  // 1. Resolve the sys_ui_related_list parent row (table + view + global sys_user).
  var list: any = null;
  if (!viewMissing) {
  ...
  // 3. Diff desired related lists against existing rows.
  var plan = diffChildren(relatedLists, existing, prune);
