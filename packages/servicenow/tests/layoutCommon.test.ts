...
describe("encodeQueryValue", function () {
  it("passes clean values through", function () {
    expect(encodeQueryValue("x_cadso_automate_audience")).toBe("x_cadso_automate_audience");
  });
  it("throws on comma, caret, or equals", function () {
    expect(function () { encodeQueryValue("a,b"); }).toThrow(/Invalid character/);
    expect(function () { encodeQueryValue("a^b"); }).toThrow(/Invalid character/);
    expect(function () { encodeQueryValue("a=b"); }).toThrow(/Invalid character/);
  });
});
...
describe("diffChildren", function () {
  ...
  it("creates all when nothing exists", function () {
    ...
    expect(plan.map(function (p) { return p.action; })).toEqual(["create", "create", "create"]);
    expect(plan.map(function (p) { return p.position; })).toEqual([0, 1, 2]);
  });

  it("reports unchanged when existing matches the desired order", function () {
    var plan = diffChildren(["a", "b"], [ex("a", 0), ex("b", 1)], true);
    expect(plan.map(function (p) { return p.action; })).toEqual(["unchanged", "unchanged"]);
  });
  ...
  it("deletes extras when prune is true", function () {
    ...
    var del = plan.filter(function (p) { return p.action === "delete"; });
  ...
  it("keeps and repositions extras when prune is false", function () {
    ...
    expect(plan.filter(function (p) { return p.action === "delete"; })).toHaveLength(0);
    var old = plan.filter(function (p) { return p.key === "old"; })[0];
  ...
  it("mixes unchanged + create + delete", function () {
    ...
    expect(plan[1]).toMatchObject({ action: "create", key: "new", position: 1 });
...
describe("assertUpdateSet", function () {
  it("rejects an empty sys_id", async function () {
    ...
    await expect(assertUpdateSet(ctx.client, "")).rejects.toThrow(/updateSetSysId is required/);
  });
  it("rejects a missing update set", async function () {
    var ctx = makeMockClient({ query: async function () { return []; } });
    await expect(assertUpdateSet(ctx.client, "us1")).rejects.toThrow(/not found/);
  });
  it("rejects an update set that is not in progress", async function () {
    var ctx = makeMockClient({
      query: async function () { return [{ sys_id: "us1", name: "Done", state: "complete" }]; }
    });
    await expect(assertUpdateSet(ctx.client, "us1")).rejects.toThrow(/in progress/);
  });
  it("returns the ref for an in-progress update set", async function () {
    var ctx = makeMockClient({
      query: async function () { return [{ sys_id: "us1", name: "Work", state: "in progress" }]; }
    });
    expect(await assertUpdateSet(ctx.client, "us1")).toEqual({ sysId: "us1", name: "Work" });
  });
});
...
describe("resolveScope", function () {
  it("returns the explicit scope without querying", async function () {
    var ctx = makeMockClient();
    var scope = await resolveScope(ctx.client, "x_cadso_automate_audience", "x_cadso_automate");
  ...
  it("resolves the scope from sys_db_object -> sys_scope", async function () {
    var ctx = makeMockClient({
      query: async function (table: string) {
        if (table === "sys_db_object") return [{ sys_scope: { value: "scope1" } }];
        if (table === "sys_scope") return [{ scope: "x_cadso_automate", name: "Tenon" }];
        return [];
      }
    });
    expect(await resolveScope(ctx.client, "x_cadso_automate_audience")).toBe("x_cadso_automate");
  });
...
describe("resolveView", function () {
  it("resolves an empty name to the Default view", async function () {
    ...
    var v = await resolveView(ctx.client, {
      viewName: "", updateSetSysId: "us1", scope: "global", dryRun: false
    });
  ...
  it("returns an existing view without creating it", async function () {
    var ctx = makeMockClient({
      query: async function (table: string) {
        if (table === "sys_ui_view") return [{ sys_id: "v1", name: "sales_support" }];
        return [];
      }
    });
    var v = await resolveView(ctx.client, {
      viewName: "sales_support", updateSetSysId: "us1", scope: "global", dryRun: false
    });
    expect(v).toEqual({ sysId: "v1", name: "sales_support", action: "unchanged" });
    expect(ctx.calls.createRecord).toHaveLength(0);
  });
  it("creates a missing view", async function () {
    var ctx = makeMockClient({ query: async function () { return []; } });
    var v = await resolveView(ctx.client, {
      viewName: "sales_support", updateSetSysId: "us1", scope: "x_cadso_automate", dryRun: false
    });
  ...
  it("plans a create without writing under dryRun", async function () {
    var ctx = makeMockClient({ query: async function () { return []; } });
    var v = await resolveView(ctx.client, {
      viewName: "sales_support", updateSetSysId: "us1", scope: "global", dryRun: true
    });
