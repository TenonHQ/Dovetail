...
describe("copyFlow", function () {
  it("POSTs the copy endpoint with name+scope and returns the new sys_id (result.data string)", async function () {
    var ctx = mockClient({
      postResponse: { result: { data: NEW, errorMessage: "" } },
    });
    var r = await copyFlow({
      client: ctx.client,
      sourceSysId: SOURCE,
      newName: "IGNORE - copy",
      scopeSysId: SCOPE,
    ...
    expect(ctx.cap.posts[0].path).toBe(
      "/api/now/processflow/flow/" +
        SOURCE +
        "/copy?sysparm_transaction_scope=" +
        SCOPE,
    );
    expect(ctx.cap.posts[0].body).toEqual({
      name: "IGNORE - copy",
      scope: SCOPE,
    });
  ...
  it("throws when the response carries no new sys_id", async function () {
    var ctx = mockClient({
      postResponse: { result: { data: "", errorMessage: "nope" } },
    });
    await expect(
      copyFlow({
        client: ctx.client,
