...
describe("servicenow client dovetail->claude 404 fallback", function () {
  ...
  it("on 404, retries against /api/cadso/dovetail/<op> and warns once", async function () {
    ...
    expect(mockHttp.request.mock.calls[0][0].url).toBe(
      "/api/cadso/dovetail_core/currentUpdateSet",
    );
    expect(mockHttp.request.mock.calls[1][0].url).toBe(
      "/api/cadso/dovetail/currentUpdateSet",
    );
  ...
  it("after latching, subsequent calls go straight to /api/cadso/dovetail/<op>", async function () {
    ...
    await client.claude.currentUpdateSet();
    await client.claude.createRecord({
      table: "incident",
    ...
    expect(mockHttp.request.mock.calls[2][0].url).toBe(
      "/api/cadso/dovetail/createRecord",
    );
    expect(mockHttp.request.mock.calls[3][0].url).toBe(
      "/api/cadso/dovetail/pushWithUpdateSet",
    );
    expect(warnSpy).toHaveBeenCalledTimes(1);
  });
  ...
  it("does NOT fall back on 401 (auth error throws immediately)", async function () {
    ...
    await expect(client.claude.currentUpdateSet()).rejects.toThrow(
      /SN auth error 401/,
    );
  ...
  it("each createClient() call gets a fresh fallback latch (closure isolation)", async function () {
    ...
    var lastCall =
      mockHttp.request.mock.calls[mockHttp.request.mock.calls.length - 1][0];
