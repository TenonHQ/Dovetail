...
function entryRow(sysId: string, relatedList: string, position: number) {
  return { sys_id: sysId, related_list: relatedList, position: String(position) };
}
...
describe("setRelatedLists", function () {
  it("rejects an empty relatedLists array", async function () {
    ...
    await expect(setRelatedLists(ctx.client, {
  ...
  it("creates the related list and all entries from scratch", async function () {
    ...
    var listCreates = ctx.calls.createRecord.filter(function (c) { return c.table === "sys_ui_related_list"; });
    var entryCreates = ctx.calls.createRecord.filter(function (c) { return c.table === "sys_ui_related_list_entry"; });
    ...
    expect(entryCreates.map(function (c) { return c.fields.related_list; })).toEqual([
      "x_cadso_automate_email_send.audiences",
      "x_cadso_automate_sms_send.audiences",
      "REL:abc123"
    ]);
    expect(entryCreates.map(function (c) { return c.fields.position; })).toEqual(["0", "1", "2"]);
  ...
  it("is idempotent — re-running an existing layout writes nothing", async function () {
    var ctx = makeMockClient({
      query: async function (table) {
        ...
        if (table === "sys_ui_related_list_entry") return [
  ...
  it("repositions a moved entry", async function () {
    ...
    expect(ctx.calls.pushWithUpdateSet[0].table).toBe("sys_ui_related_list_entry");
    expect(ctx.calls.deleteRecord).toHaveLength(0);
  });
