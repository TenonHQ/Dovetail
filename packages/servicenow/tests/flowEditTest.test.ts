    ...
    now: {
      get: async function <T>(path: string): Promise<T> {
        ...
        if (opts.getResponses) {
          var r =
            opts.getResponses[getIdx] !== undefined
              ? opts.getResponses[getIdx]
              : opts.getResponses[opts.getResponses.length - 1];
          getIdx += 1;
          return r as T;
        }
...
describe("editFlow", function () {
  it("dry-run computes the rename + step-input diff without publishing", async function () {
    ...
    var r = await editFlow({
      ...
      ops: {
        ...
        patchStepInputs: [
          { step: "Calculate SMS Send At", input: "send_rate", value: "5" },
        ],
      },
    });
    ...
    expect(
      r.changes.some(function (c) {
        return c.indexOf("send_rate") >= 0;
      }),
    ).toBe(true);
  ...
  it("apply republishes the patched model + verifies persistence (no warning when it took)", async function () {
    ...
    var r = await editFlow({
      ...
      ops: {
        patchStepInputs: [
          { step: "calc", input: "timezone", value: "America/Denver" },
        ],
      },
    });
    ...
    var tz = calc.inputs.filter(function (x: any) {
      return x.name === "timezone";
    })[0];
  ...
  it("warns when a step-input change did not persist (snapshot POST no-op)", async function () {
    ...
    expect(
      r.warnings.some(function (w) {
        return w.indexOf("did not persist") >= 0;
      }),
    ).toBe(true);
  });
  ...
  it("metadata-only edits write the record (no snapshot recompile)", async function () {
    ...
    var ctx = mockClient({ getResponse: flowModel() });
    ctx.client.claude.pushWithUpdateSet = async function (p: any) {
      pushes.push(p);
    ...
    expect(pushes[0].update_set_sys_id).toBe(
      "us_sys_id_000000000000000000000000",
    );
  });
  ...
  it("reports unmatched steps/inputs as warnings and does not publish a no-op", async function () {
    ...
    var r = await editFlow({
      ...
      apply: true,
      ops: {
        patchStepInputs: [{ step: "NoSuchStep", input: "x", value: "1" }],
      },
    });
  ...
  it("requires sysId", async function () {
    var ctx = mockClient({ getResponse: flowModel() });
    await expect(
      editFlow({ client: ctx.client, sysId: "", ops: {} }),
...
describe("testFlow", function () {
  ...
  it("execute requires confirm=true", async function () {
    var ctx = mockClient({ getResponse: flowModel() });
    await expect(
      testFlow({ client: ctx.client, sysId: FLOW, mode: "execute" }),
  ...
  it("execute with confirm POSTs the runner endpoint and returns the context id", async function () {
    var ctx = mockClient({
      postResponse: { result: { contextId: "ctx-123", outputs: { ok: true } } },
    });
    var r = await testFlow({
      client: ctx.client,
      sysId: FLOW,
  ...
  it("requires sysId", async function () {
    var ctx = mockClient({ getResponse: flowModel() });
    await expect(testFlow({ client: ctx.client, sysId: "" })).rejects.toThrow(
      /sysId is required/,
    );
  });
});
