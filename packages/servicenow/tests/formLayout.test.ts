...
describe("setFormLayout", function () {
  ...
  it("creates a single-section form with all its fields from scratch", async function () {
    ...
    var formCreates = ctx.calls.createRecord.filter(function (c) {
      return c.table === "sys_ui_form";
    });
    var sectionCreates = ctx.calls.createRecord.filter(function (c) {
      return c.table === "sys_ui_section";
    });
    var joinCreates = ctx.calls.createRecord.filter(function (c) {
      return c.table === "sys_ui_form_section";
    });
    var elementCreates = ctx.calls.createRecord.filter(function (c) {
      return c.table === "sys_ui_element";
    });
  ...
  it("creates a two-section form with positioned join rows", async function () {
    ...
    expect(
      sectionCreates.map(function (c) {
        return c.fields.caption;
      }),
    ).toEqual(["", "Meta Data"]);
  ...
  it("is idempotent — re-running an existing layout writes nothing", async function () {
    var ctx = makeMockClient({
      query: async function (table, query) {
        ...
        if (table === "sys_ui_section")
          return [sectionRow("sec0", ""), sectionRow("sec1", "Meta Data")];
        if (table === "sys_ui_form_section") {
          return [
            joinRow("j0", "form1", "sec0", 0),
            joinRow("j1", "form1", "sec1", 1),
          ];
        }
  ...
  it("reorders fields within a section via pushWithUpdateSet", async function () {
    var ctx = makeMockClient({
      query: async function (table, query) {
        ...
        if (table === "sys_ui_form_section")
          return [joinRow("j0", "form1", "sec0", 0)];
  ...
  it("prunes a dropped section, its join, and all its element rows", async function () {
    var ctx = makeMockClient({
      query: async function (table, query) {
        ...
        if (table === "sys_ui_element") {
          ...
          if (query === "sys_ui_section=sec1") {
            return [
              elementRow("e1", "created_by", 0),
              elementRow("e2", "sys_updated_on", 1),
            ];
          }
          return [];
        }
    ...
    var deletedTables = ctx.calls.deleteRecord.map(function (c) {
      return c.table;
    });
    expect(
      deletedTables.filter(function (t) {
        return t === "sys_ui_form_section";
      }),
    ).toHaveLength(1);
    expect(
      deletedTables.filter(function (t) {
        return t === "sys_ui_section";
      }),
    ).toHaveLength(1);
    expect(
      deletedTables.filter(function (t) {
        return t === "sys_ui_element";
      }),
    ).toHaveLength(2);
    ...
    var elementIdxs = ctx.calls.deleteRecord
      .map(function (c, i) {
        return c.table === "sys_ui_element" ? i : -1;
      })
      .filter(function (i) {
        return i >= 0;
      });
    elementIdxs.forEach(function (i) {
      expect(i).toBeLessThan(sectionIdx);
    });
    expect(
      result.records.filter(function (r) {
        return r.action === "deleted";
  ...
  it("adds a new section to an existing single-section form", async function () {
    ...
    expect(joinCreates[0].fields.sys_ui_section).toBe(
      sectionCreates[0] && "new_1",
    );
