...
Consuming projects wire the server in via their `.mcp.json`: `npx -y @tenonhq/dovetail-claude-plans mcp`.
...
- **Modes:** _dry-run_ (default) resolves the spawn command + working dir, appends a `dry-run` `DispatchEvent` to `dispatch_log`, and returns — **no process is launched, and the token is not consumed**. _Live_ (`confirm: true` + valid `token`) consumes the plan's outstanding token **atomically before** spawning, so a crashed/leaked subprocess never breaks the single-use guarantee.
...
1. **`set_stage`** advances the plan through the 10-stage state machine and hands back a **one-time dispatch token** (5-minute TTL). The token is the _only_ way to drive a live spawn, and it is rotated on every `set_stage`.
  ...
  "sections": [
    {
      "type": "header",
      "title": "Deploy PR #42",
      "subtitle": "feature/auth → PROD"
    },
    {
      "type": "meta",
      "rows": [{ "label": "Status", "value": "Approved", "badge": "success" }]
    },
    {
      "type": "steps",
      "steps": [
