...
export function scorePlanFeatures(input: ScoreInput): FeatureScore {
  ...
  var hasStructured = typeof plan.content_html === "string" && plan.content_html.length > 0;
  var bodyLen = (plan.content_md || "").length + (plan.content_html || "").length;
  var hasBody = bodyLen >= BODY_FLOOR;
  var hasDiagram = artifacts.some(function (a) { return a.kind === "mermaid"; });
  var hasDoc = artifacts.some(function (a) { return a.kind === "markdown"; });
  var hasLinks = Array.isArray(plan.linked_artifacts) && plan.linked_artifacts.length > 0;
  var hasPr = plan.pr_url !== undefined || plan.pr_number !== undefined;
  var hasCategories = Array.isArray(plan.categories) && plan.categories.length > 0;

  var dims = [
    { ok: hasStructured, weight: 0.15, label: "structured content (content_structured / content_html)" },
    { ok: hasBody, weight: 0.15, label: "a substantial body (>= " + BODY_FLOOR + " chars)" },
    { ok: hasDiagram, weight: 0.2, label: "at least one diagram (push_diagram)" },
    { ok: hasDoc, weight: 0.15, label: "at least one markdown artifact (push_artifact)" },
    { ok: hasLinks, weight: 0.1, label: "linked related plans (linked_artifacts)" },
  ...
  if (missing.length === 0) {
    ...
    var ranked = dims
      .filter(function (d) { return !d.ok; })
      .sort(function (a, b) { return b.weight - a.weight; })
      .slice(0, 2)
      .map(function (d) { return d.label; });
    hint =
      "Score " + score.toFixed(2) + "/1.00 — add " + ranked.join(" and ") + " to raise it.";
  }
